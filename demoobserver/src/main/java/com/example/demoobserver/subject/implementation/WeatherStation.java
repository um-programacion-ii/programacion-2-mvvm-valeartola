...
public class WeatherStation implements Subject<WeatherData> {
    ...
    public void setMeasurements(float temperature, float humidity, float pressure) {
        log.info("Actualizando mediciones - Temp: {} ºC, Humedad: {} %, Presion: {} hPa", temperature, humidity, pressure);
        this.currentWeatherData = new WeatherData(temperature, humidity, pressure);
        measurementsChanged();
    }

    public void setMeasurements(float temperature, float humidity) {
        log.info("Actualizando mediciones - Temp: {}ºC, Humedad: {} %", temperature);
        this.currentWeatherData = new WeatherData(temperature, humidity);
        measurementsChanged();
    }

    private void measurementsChanged() {
        log.info("Notificando a observadores de los cambios");
        notifyObservers();
    }
